...
const int DISTANCE_THRESHOLD = 30;  // Adjust based on your ultrasonic sensor readings (in cm)
const int PUSH_SPEED = 100;      // Maximum speed for pushing
const int SEARCH_SPEED = 100;    // Speed while searching
...
void loop() {
  ...
  if (frontDistance < DISTANCE_THRESHOLD) {  // Ultrasonic sensor: closer distance means smaller value
    ...
    // No box detected - search by rotating
    Serial.println("Action: No box detected, rotating to search");
    ROTATE(SEARCH_SPEED);
  }
}
